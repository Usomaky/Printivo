...
const UploadModal = ({ showmodal, modalHandler }) => {
  ...
  const { updateCurrentProduct, currentProduct, addToCart } = useProducts();
  ...
  const uploadFile = (autoUploadFile = null) => {
    if (!autoUploadFile) {
      if (file) {
      ...
      autoUploadFile.forEach((image, i) => {
        singleUpload(image, autoUploadFile.length);
      });
    }
  };

  const singleUpload = async (image, autoUploadFileLength) => {
    ...
    try {
      ...
      count++;
      if (count === file?.length || count === autoUploadFileLength) {
        setIsDisabled(false);
        setIsUploading(false);
        updateCurrentProduct({
          ...currentProduct,
          uploadUrl: uploadData,
          item_type: "custom-upload",
          designType: "Custom file upload",
        });
        setButtonText("Proceed to Cart");
      }
    } catch (error) {
      console.log(error);
      setIsUploading(false);
  ...
  return (
    <div
      className={`c-modal c-modal--upload px-3 ${showmodal ? "active" : ""}`}
    >
      <div className="inner w-full bg-white">
        <div className="c-modal--upload__header w-full flex justify-between items-center py-2 px-6">
        ...
        <div className="c-custom__upload">
          {file === null ? (
            <div
              {...getRootProps()}
              className={`c-modal--upload__body bg-pale-brown p-6 ${
                isDragActive ? "inactive" : ""
              }`}
            >
        ...
        <div className="c-modal--upload__footer py-3 px-6 text-right">
          <button
            disabled={isDisabled}
            onClick={goToCart}
            className="text-sm font-sf text-white bg-red-light px-6 py-3 flex items-center ml-auto rounded-sm"
          >
            {buttonText} {isUploading && <Loader />}
          </button>
        </div>
      </div>
    </div>
