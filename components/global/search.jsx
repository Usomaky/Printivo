...
export default function Search({ placeholder, query, handleResult }) {
  ...
  return (
    <div
      className={`search-con hidden md:block relative ${
        queryText ? "max-w-md" : "max-w-xs"
      } -mt-3 ml-auto`}
    >
      <form className="c-form--search" onSubmit={onSearchSubmit}>
        <input
      ...
      <div
        ...
        className={`${
          queryText?.length > 0 ? "hidden md:block shadow-lg" : "hidden"
